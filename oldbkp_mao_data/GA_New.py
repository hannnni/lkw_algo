...
class ga:
        ...
        def search_old_solutions(self,HGVInputs):
            ...
            if len(pickleObjs)>0:
                for i in range(len(pickleObjs)):
                    simil = 0
                    for j in range(len(pickleObjs[i][1])):
        ...
        def crossover(self, mum, dad):
                 ...
                 if (random.uniform(0, 1) > self.crossover_rate):
                         ...
                         baby2 = dad.bits;
                         return baby1, baby2
                 cp = random.randint(0, self.genome_len - 1)#crossover point selection, for our case need to be restricted
...
#thinking about one or two crossover point?
        ...
        def mutate(self, bits):#considering the same prosilility to be mutated(for all the programms), one position or morem positions mutation?
...
##                                 self.fittest_genome = self.genomes[i]
        ...
        def epoch(self, overallSignalProgramms,vehcleInputs,HGVInputs):
             ...
             while (new_babies < self.pop_size):
                     ...
                     dad = self.selection()
                     MaxiRate = int(self.genome_len/int(math.sqrt(NoOfSignalProgramms)))
